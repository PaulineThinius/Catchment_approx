...
def compute_climatological_parde(df, alpakas_id, obs_col="Obs"):
    ...
    Q_annual =  df[obs_col].resample("ME").mean().mean()
...
def cluster_parde_springs(all_parde_df, features, n_clusters=4, approaches=["kmeans", "hierarchical"],
                          save_dir="./plots", save_csv="./results/parde_coefficent_cluster.csv"):
    """
...
alpakas_ids = pd.read_csv('alpakas_ids.csv', header=None)[0].tolist()
catch_representations = ["expert", "approx", "point"]
...
for alpakas_id in alpakas_ids:
    ...
    # --------------------------------------------------------
    reference_path = (
        f"./results/{static_feature_types[0]}/"
...
all_scores_df.to_csv("./results/all_scores_median.csv")
