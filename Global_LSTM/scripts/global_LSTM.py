...
pth_dt_dyn = "../preprocessed_data/dynamic"
base_out = f"../output_data/{mode_features}/{mode_aggregate}"
...
if mode_features == "static":
    dt_static_features = pd.read_csv(f'../preprocessed_data/static/{mode_aggregate}_stat_attrs.csv')
# Random features are the same for each catchment representation
elif mode_features == "random":
    dt_static_features = pd.read_csv(f'../preprocessed_data/static/random_attrs.csv')
    if mode_aggregate == "point":
        dt_static_features = dt_static_features.drop(columns=["elev_min", "elev_max"])
else:
    dt_static_features = pd.read_csv(f'../preprocessed_data/static/time_series_attrs.csv')
